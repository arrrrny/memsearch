...
|---------|:---:|:---:|:---:|:---:|:---:|
| **Plugin type** | Shell hooks | TS registerTool | TS npm plugin | Shell hooks | SKILL.md + scripts (hookless) |
| **Capture method** | Stop hook (async) | agent_end hook | SQLite daemon | Stop hook (async) | SQLite poller (cron) |
| **Summarization** | `claude -p --model haiku` | OpenClaw agent | `opencode run` | `codex exec` | `hermes-capture.py` (state.db) |
| **Recall mechanism** | SKILL.md (context: fork) | memory_search tool | memory_search tool | SKILL.md | SKILL.md |
...
| Hermes Agent user | [Hermes plugin](hermes/index.md) -- hookless, SKILL.md recall + state.db capture |
